...
export function CourseCard({ course }: CourseCardProps) {
  ...
  const handleConfirm = (name: string, email: string) => {
    setModalOpen(false)
    const result = enrollCourse(course.id, name, email)
    if (result.success) {
      showToast(
        `You're enrolled in "${course.title}"! Simulated payment of ${formatCurrency(course.price)} confirmed.`,
        'success',
      )
    } else if (result.reason === 'duplicate') {
      showToast(`${email} is already enrolled in this course.`, 'error')
    } else {
  ...
  return (
    <>
      ...
      <RegistrationModal
        ...
        price={course.price}
        confirmLabel={`Enroll — ${formatCurrency(course.price)}`}
